//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
//  'Tiny 13A led sensor AirWick' v.1.4.2 light by Serg7461 
	...
	while(1)
	{
		...
		{			
			...
			if (mode > 0 && main_timer >= motor_on_time[((mode-1)%3)])
			{
				...
				if (mode < 8 && (!light_on_flag || light_timer >= MAX_LIGHT_TIME)) { motor_work(); }
				// Если режим 8-10, то пшикаем через время общего таймера при вкл. свете
				if ( 7 < mode && mode < 11 && light_on_flag ) { motor_work(); }
				// Если режим 11-13, то пшикаем через время общего таймера без света
				if ( 10 < mode && !light_on_flag ) { motor_work(); }
				main_timer = 0;
			}else{
