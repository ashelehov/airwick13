//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
//  'Tiny 13A led sensor AirWick' v.1.4.4 light by Serg7461
...
#define MOTOR_WORK_TIME   60            // продолжительность работы мотора, миллисекунд, БЕЗ МНОЖИТЕЛЯ!
...
#define BLINK           2           // 2 раза мигаем в туалете
#else
...
#define BLINK           3           // 3 раза мигаем вне туалета
#endif
  ...
  while (1)
  {
    ...
    if (!sleep_flag)
    {
      ...
      if (mode > 0 && main_timer >= motor_on_time[((mode - 1) % 3)])
      {
        ...
        if (mode < 4 && (!light_on_flag || light_timer >= MAX_LIGHT_TIME)) {
          motor_work();
        }
        // Если режим 4-6, то пшикаем через время общего таймера игнорируя свет
        if ( 3 < mode && mode < 7 ) {
          motor_work();
        }
        // Если режим 7-9, то пшикаем через время общего таймера при вкл. свете
        if ( 6 < mode && mode < 10 && light_on_flag ) {
          motor_work();
        }
        // Если режим 10-12, то пшикаем через время общего таймера без света
        if ( 9 < mode && !light_on_flag ) {
          motor_work();
        }
        main_timer = 0;
      } else {
      ...
      // Если свет включен
      if (light_on_flag) {
        led_blink(BLINK, 10);       // Со светом мигаем 3 раза в обычном режиме, 2 - в туалете
        light_timer++;
...
// 0) Пшикаем только по датчику света
// 1) Пшикаем 1 раз в час, минимальное время включения света - 2.5 минуты
// 2) -//- каждые 3 часа,
// 3) -//- каждые 6 часов.
// 4) Пшикаем 1 раз в час, игнорируя датчик света
// 5) -//- каждые 3 часа,
// 6) -//- каждые 6 часов.
//
